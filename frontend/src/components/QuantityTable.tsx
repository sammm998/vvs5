...
export function withFloorHeight(rows: any[], floorHeight: number | null): any[] {
  ...
  return rows.map((r) => {
    ...
    return { ...r, vertical_calc: v, total_calc: r.confirmed_horizontal_m + (v ?? 0) };
  });
}

export default function QuantityTable({ rows, selected, onSelect, floorHeight }: { rows: any[]; selected: string | null; onSelect: (key: string | null) => void; floorHeight: number | null }) {
  const [q, setQ] = useState("");
  const [status, setStatus] = useState("");
  const [sort, setSort] = useState<{ k: string; dir: 1 | -1 }>({ k: "designation", dir: 1 });
  const list = useMemo(() => {
    let l = withFloorHeight(rows, floorHeight).filter((r) => (!q || r.designation.toLowerCase().includes(q.toLowerCase()) || String(r.dn).includes(q)) && (!status || r.state === status));
    l = [...l].sort((a, b) => { const va = a[sort.k], vb = b[sort.k]; return (va > vb ? 1 : va < vb ? -1 : 0) * sort.dir; });
    return l;
  }, [rows, q, status, sort, floorHeight]);
