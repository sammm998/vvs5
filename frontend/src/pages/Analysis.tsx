...
export default function AnalysisPage() {
  ...
  const [layers, setLayers] = useState<Record<Layer, boolean>>({ pipes: true, ambiguous: true, unowned: false, designations: false, leaders: false, anchors: false });
    ...
    <div className="analysis">
      <div className="left">
        ...
        <PdfViewer ref={viewer} data={pdf} page={page} pipes={pipesOnPage} ambiguous={result.ambiguous_geometry} unowned={result.unowned_geometry}
          designations={result.designations} leaders={result.leaders} anchors={result.anchors} selectedIdentity={selIdent}
      ...
      <div className="right">
        ...
        {tab === "mangder" && (
          <div className="card">
            ...
            <QuantityTable rows={result.quantities} selected={selIdent} onSelect={(k) => { setSelIdent(k); setSelPipe(null); setWhy(null); }} />
            {why && (
        ...
        {tab === "oversikt" && (
          <div className="card">
            ...
            <p style={{ marginTop: 12 }}>Skala: <b>{result.scale.state}</b> ({result.scale.reason}) · Reconciliation: <b>{c.reconciliation}</b> · Determinism: <b>{c.determinism ?? "ej körd"}</b> · Contamination: <b>{c.contamination}</b></p>
        ...
        {tab === "artefakter" && (
          <div className="card">
            ...
            <div className="row">
              ...
              <button onClick={() => dl(api.exportUrl(id!, "xlsx"), "mangder.xlsx")}>Excel</button>
              <button onClick={() => dl(api.exportUrl(id!, "csv"), "mangder.csv")}>CSV</button>
